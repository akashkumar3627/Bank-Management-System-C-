...
void Updocion_bank_account(int); //function to updoce account details
...
int main(){
    ...
    switch(ch){
        ...
        case '7':
            ...
            cout<<"\t Enter the account number: ";
            cin>>num;
            Updocion_bank_account(num);            //modify account function
            break;
        case '8':
...
void write_account() //function to write record in binary file
{
    Bank_Account ac;
    ofstream outFile;
    outFile.open("account.doc",ios::binary|ios::app);
    ac.create_account();
...
void Updocion_bank_account(int n){
    bool found = false;
    Bank_Account ac;
    fstream File;
    File.open("account.doc", ios::binary|ios::in|ios::out); //open the file in binary mode
    if(!File){
        cout<<"File could not be open !! Press any key...";
        return;
    }
